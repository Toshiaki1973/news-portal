...
- ワークフロー: `.github/workflows/deploy.yml`。毎時22分（[[feedback-cron-minute-convention]]の慣習で00分を避ける）+ `workflow_dispatch`（手動実行）+ mainへのpushをトリガーに、`fetch_data.py`実行→`output/`をPagesにデプロイ
